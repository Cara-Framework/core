...
def test_notification_mail_adds_confirmation_link_and_rfc8058_headers() -> None:
    values = {
    ...
    assert (
        MailChannel(manager, link_settings=values).send(notifiable, _Notification())
        is True
    )
...
def test_notification_mail_requires_an_opaque_public_recipient_id() -> None:
    values = {
        "app.frontend_url": "https://app.example",
        "app.preferences_url": "https://app.example/notifications/preferences",
        "app.unsubscribe_confirm_url": "https://app.example/unsubscribe",
    ...
    with pytest.raises(ConfigurationException, match="notifiable.public_id"):
        MailChannel(manager, link_settings=values).send(notifiable, _Notification())


def test_visible_unsubscribe_falls_back_to_the_processor_without_a_confirm_page() -> None:
...
def test_one_click_is_dropped_when_the_notification_supplies_its_own_link() -> None:
    ...
    assert (
        MailChannel(manager, link_settings=values).send(
            notifiable, _OverridingNotification()
        )
        is True
    )
    view_data = manager.message.view_data
