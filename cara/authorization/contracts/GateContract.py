...
class GateContract(ABC):
    """Contract for managing authorization checks."""

    # -- configuration ----------------------------------------------------- #

    @abstractmethod
    def define(self, ability: str, callback: Callable | str) -> GateContract:
        """Define a standalone ability."""

    @abstractmethod
    def policy(self, model_class: Any, policy_class: Any) -> GateContract:
        """Bind a policy class to a model class."""

    @abstractmethod
    def register_policies(self, policies: list[tuple]) -> GateContract:
        """Bind multiple ``(Model, Policy)`` tuples."""

    @abstractmethod
    def before(self, callback: Callable) -> GateContract:
        """Register a pre-check callback that may short-circuit."""

    @abstractmethod
    def after(self, callback: Callable) -> GateContract:
    ...
    @abstractmethod
    def for_user(self, user: Any) -> GateContract:
