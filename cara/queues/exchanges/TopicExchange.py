    ...
    def dispatch_job(self, routing_key: str, job_instance, payload: Optional[Dict] = None) -> Optional[str]:
        ...
        try:
            ...
            if not matching_queues:
                # FAIL LOUD instead of returning None. A routing key with no
                # binding means the message would be black-holed by the broker
                # and the calling code would assume success. Raising here
                # surfaces the misconfiguration immediately (in tests, in CI,
                # and at runtime via the queue runner's retry/failure path)
                # rather than letting work silently disappear.
                msg = (
