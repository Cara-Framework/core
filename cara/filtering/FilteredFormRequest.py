"""``FilteredFormRequest`` — FormRequest with FilterSet support.
...
from .SortRegistry import SortRegistry
...
class FilteredFormRequest(FormRequest):
    ...
    relations: ClassVar[RelationSet] = RelationSet()
    ...
    def rules(self) -> dict:
        ...
        if self.sort_registry is not None:
            rules.setdefault("sort_by", self.sort_registry.validation_rule())
        return rules
    ...
    async def validate_request(self, request) -> dict:
        ...
        # Step 6 — resolve the single public sort parameter.
        sort_name = validated.get("sort_by") if self.sort_registry is not None else None
        if (not sort_name) and self.default_sort and self.sort_registry is not None:
            sort_name = self.default_sort
            validated["sort_by"] = sort_name
