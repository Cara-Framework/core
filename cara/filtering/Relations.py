...
::
    ...
    Model.active().with_(["images", "price", "parent"])
    Model.active().with_(["images", "price", "parent", "details", "videos"])
    Model.active().with_(["images", "price"])

Twenty-plus call sites, three subtly different shapes. Adding a
relation to "the card payload" required a global grep + edit; one
surface loaded a different shape than another for no business
reason; a repository hard-coded its own list and silently fell
behind when ``price`` was added.
...
::
    ...
    from cara.filtering import RelationSet

    # An entity card — minimum needed for a resource to render an
    # image, a price, and a parent link.
    CARD_RELATIONS = RelationSet(
        "images",
        "price",
        "parent",
        name="entity.card",
    ...
    # An entity detail — card + the heavy fields needed for the
    # show / detail endpoint.
    DETAIL_RELATIONS = CARD_RELATIONS.with_(
        "details",
        "videos",
        name="entity.detail",
    )
...
class RelationSet(tuple):
        ...
        Args:
            *relations: Relation names to append.
            name: Optional name for the derived set. Empty by
                ...
                ("entity.detail") so debug output and the wizard
    ...
    def without(self, *drop: str, name: str = "") -> RelationSet:
        ...
        ``DETAIL_RELATIONS.without("videos")``).
        """
        skip = set(drop)
        return RelationSet(
...
def relations(*items: str, name: str = "") -> RelationSet:
    """Sugar constructor — ``relations("images", "price")``.
