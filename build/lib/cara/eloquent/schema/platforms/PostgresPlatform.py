...
class PostgresPlatform(Platform):
    ...
    def compile_create_sql(self, table, if_not_exists=False):
        sql = []
        table_create_format = (
            self.create_if_not_exists_format() if if_not_exists else self.create_format()
        )
        sql.append(
    ...
    def compile_alter_sql(self, table):
        ...
        if table.renamed_columns:
            ...
            for (
                name,
                column,
            ) in table.get_renamed_columns().items():
                if column.length:
                    length = self.create_column_length(column.column_type).format(
                        length=column.length
                    )
        ...
        if hasattr(table, "dropped_columns") and table.dropped_columns:
            dropped_sql = []

            for name in table.get_dropped_columns():
                dropped_sql.append(
                    self.drop_column_string().format(name=self.wrap_column(name)).strip()
                )
        ...
        if hasattr(table, "changed_columns") and table.changed_columns:
            changed_sql = []

            for (
                name,
                column,
            ) in table.changed_columns.items():
                column_constraint = ""
                if column.column_type == "enum":
                    values = ", ".join(f"'{x}'" for x in column.values)
                    column_constraint = f" CHECK({column.name} IN ({values}))"
                ...
                if column.default_value is not None:
                    changed_sql.append(
                        f"ALTER COLUMN {self.wrap_column(name)} SET DEFAULT {column.default_value}"
                    )
        ...
        if hasattr(table, "removed_indexes") and table.removed_indexes:
            constraints = table.removed_indexes
            for constraint in constraints:
                sql.append(f"DROP INDEX {constraint}")

        if (
            (hasattr(table, "dropped_foreign_keys") and table.dropped_foreign_keys)
            or (hasattr(table, "removed_unique_indexes") and table.removed_unique_indexes)
            or (hasattr(table, "dropped_primary_keys") and table.dropped_primary_keys)
        ):
            constraints = getattr(table, "dropped_foreign_keys", [])
            constraints += getattr(table, "removed_unique_indexes", [])
            constraints += getattr(table, "dropped_primary_keys", [])
            for constraint in constraints:
                sql.append(
    ...
    def modify_column_string(self):
        return (
            "ALTER COLUMN {name} TYPE {data_type}{length}{column_constraint} {constraint}"
        )
    ...
    def constraintize(self, constraints, table):
        ...
        for name, constraint in constraints.items():
            sql.append(
                getattr(
                    self,
                    f"get_{constraint.constraint_type}_constraint_string",
                )().format(
    ...
    def get_current_schema(self, connection, table_name, schema=None):
        ...
        for column in result:
            ...
            # find default
            default_val = column.get("dflt_value", "") or column.get("column_default", "")
            if default_val and default_val.startswith("nextval"):
