...
class BaseTestCaseSelectGrammar:
    ...
    def test_can_compile_with_max_and_columns_different_order(
        self,
    ):
    ...
    def test_can_compile_having_with_greater_than_expression(
        self,
    ):
        to_sql = self.builder.sum("age").group_by("age").having("age", ">", 10).to_sql()
        sql = getattr(
    ...
    def test_can_compile_right_join_clause_with_lambda(
        self,
    ):
    ...
    def test_can_compile_where_raw_and_where_with_multiple_bindings(
        self,
    ):
