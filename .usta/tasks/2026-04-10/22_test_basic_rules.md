## Add test coverage for EmailRule, URLRule, and BooleanRule

These three validation rules have zero test coverage. Key details from source: `BooleanRule` accepts `"yes"`/`"no"` as valid and overrides `message()` directly. `EmailRule` and `URLRule` override `default_message()`. All `validate()` methods return `bool(...)` so `assert x is True` is safe here.
