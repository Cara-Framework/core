## Goal
Create `tests/support/test_str_utils.py` covering untested Str utilities: `random_string`, `modularize`, `as_filepath`, `removeprefix`, `removesuffix`, `match`, `add_query_params`.

## Steps
1. Create `tests/support/test_str_utils.py` with the exact content shown below.
