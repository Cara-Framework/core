## Goal
Create `tests/support/test_time.py` covering `humanize_seconds` and `format_duration` which have zero test coverage.

## Steps
1. Create `tests/support/test_time.py` with the exact content shown below.
2. The file tests both functions with zero input, singular units, plural units, mixed components, and skipped-zero components.
