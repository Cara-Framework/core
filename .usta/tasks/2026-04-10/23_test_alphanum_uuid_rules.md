## Add test coverage for AlphanumRule and UuidRule

Both rules have zero test coverage. Both override `message()` directly (not `default_message()`). Both coerce non-string values via `str()` before matching. All `validate()` returns `bool(...)` so `is True`/`is False` is safe.

### tests/validation/test_alphanum_uuid_rules.py
