## Add comprehensive tests for Event dispatcher

Test listener registration, dispatching, ordering, wildcard matching, propagation stopping, and the UserRegisteredEvent. Mock `cara.context.ExecutionContext.is_sync()` to return True so dispatch runs synchronously via `await`.
