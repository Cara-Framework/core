...
`cara/validation/MessageFormatter.py` has zero test coverage. Add a focused test file covering its four static methods with their actual signatures. Note: `has_custom_message(params)` takes a single dict arg and returns a truthy/falsy value (not necessarily literal `True`/`False`). Use `assert`/`assert not` instead of `assert x is True`.
